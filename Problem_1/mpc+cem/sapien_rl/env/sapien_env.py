...
class SapienEnv(Env):
    ...
    # this is only used for default obs, if you want another obs, use render()
    def get_obs(self, **kwargs):
        if self.obs_mode == "state":
            ...
            return {
                "agent": self.agent.get_state(observer=self.observer),
                "additional_task_info": self.get_additional_task_info(obs_mode=self.obs_mode, observer=self.observer),
                # dict of partial point clouds
                "pointcloud": self.render(mode="pointcloud", camera_names=[self.observer], **kwargs),
            }
    ...
    def get_pad_object_state(self, observer):
        base_pos, base_quat, base_vel, base_ang_vel, qpos, qvel = self.get_object_state(
            observer=observer)
        k = len(qpos)
        ...
        if mode == "human":
            ...
            if mode == "rgb_array":
                ...
                for cam in cameras:
                    ...
                    if depth:
                        img = np.concatenate(
                            [img, cam.get_float_texture("GbufferDepth")[..., None]], -1)
                    if normal:
                        img = np.concatenate(
                            [img, cam.get_float_texture("Normal")], -1)
                    if seg:
                ...
                for cam in cameras:
                    ...
                    pcds[cam.get_name()] = np.concatenate(
                        [pcd[mask], img[mask], seg[mask][..., None]], -1)
                return pcds
