...
class OpenCabinetEnv(SapienEnv):
    ...
    def _place_gripper(self):
        ...
        for scaled_vertices, _, shape_pose in shapes:
            mat = (self.target_link.pose *
                   shape_pose).to_transformation_matrix()
        ...
        dist = self._level_rng.uniform(
            low=self.gripper_init_dist[0], high=self.gripper_init_dist[1])
        delta = sample_on_unit_sphere(self._level_rng) * dist
        delta[0] = -abs(delta[0])
        ref_point_on_handle = np.array(
            [mins[0]-0.05, handle_center[1], handle_center[2]])
        ...
        current_gripper_rot_mat = Rotation.from_euler(
            'ZYX', xyz_joint_rotation).as_matrix()  # (3, ), intrinsic

        rotation_from_x_axis_to_vec_to_handle = rotation_between_vec(
            np.array([1, 0, 0]), handle_center - gripper_pos)
        # align x-axis with vector points to handle
        current_gripper_rot_mat = current_gripper_rot_mat @ rotation_from_x_axis_to_vec_to_handle.as_matrix()
        ...
        perturb_rotation = rotation_between_vec(np.array(
            [0, 0, -1]), orientation_z_half_sphere)  # negative z points outside of gripper
        # print('perturb angle:', np.linalg.norm(perturb_rotation.as_rotvec()) / np.pi * 180)
        # perturb a bit, <90 deg with x-axis
        current_gripper_rot_mat = current_gripper_rot_mat @ perturb_rotation.as_matrix()

        qpos[3:6] = Rotation.from_matrix(
            current_gripper_rot_mat).as_euler('ZYX')  # (3, ), intrinsic
    ...
    def set_state(self, state, observer='world'):
        if observer == 'world':
            ...
            self.cabinet.set_qvel(
                state[self.obj_max_dof:self.obj_max_dof+self.cabinet.dof])

            # set robot state
            task_info_len = len(
                self.get_additional_task_info('state', 'world'))
    ...
    def get_custom_observation(self):
        ...
        target_handle_points = apply_pose_to_points(
            np.asarray(target_handle_pcd.points), actor.get_pose())
        handle_position = np.mean(target_handle_points, axis=0)
        # print(handle_position.shape)

        # pos of active joints
        object_pose = self.articulations["cabinet"]["articulation"].get_qpos()
        object_vel = self.articulations["cabinet"]["articulation"].get_qvel()

        ee_relative_coords = np.concatenate(
            ee_coords - handle_position, axis=0)

        dist_ee_actor = np.array(np.sqrt(
            ((ee_coords[:, None] - target_handle_points[None]) ** 2).sum(-1)).min(-1).mean()).reshape(1)

        obj = self.articulations["cabinet"]["articulation"]
        finish_sign = np.array(obj.get_qpos()[
                               self.target_index_in_active_joints] >= self.target_qpos).reshape(1)
        ...
        # Taken from
        # https://stackoverflow.com/questions/39818733/create-dictionary-where-keys-are-variable-names/39818967
        ret = {k: v for k, v in locals().items() if not k.startswith('__')}
    ...
    def compute_reward(self, action, state=None):
        ...
        si = info_dict['state_info']

        open_enough = self.cabinet.get_qpos(
        )[self.target_index_in_active_joints] >= self.target_qpos

        # If the qpos of cabinet is greater than target qpos...
        if open_enough:

            # then we penalize the velocity.
            reward = - si['link_vel'] - si['link_ang_vel'] + 5
        ...
        ee_relative_coords = custom_obs['ee_relative_coords']

        # 2 for the 2 fingers, 3 for relative coords
        ee_relative_coords = ee_relative_coords.reshape(2, 3)

        # 1 because we only use 1 finger
        ee_distance_to_object = np.linalg.norm(ee_relative_coords[1])

        # If the ee is too far from handle...
        if ee_distance_to_object > 0.025:

            # encourage it to go near the handle.
            reward = - ee_distance_to_object

        # Otherwise, try to open the door
        else:

            reward = custom_obs['object_pose'] - si['target_qpos'] + 2
