...
class MPC:
    def __init__(self, env, plan_horizon=4, popsize=200, num_elites=20, max_iters=4, use_mpc=True):
        ...
        assert use_mpc is False

        # setting the numbeer of iterations to 1
        assert max_iters == 1

        # population size M to M * I
        assert popsize == 200 * 4

        # number of elites to 1
        assert num_elites == 1
    ...
    def act(self, state, t):
        ...
        if self.use_mpc == False:
            ...
            # obtain the t-th action in the mean action sequence
            ad = self.action_dim

            a_index = t % self.plan_horizon
